...
def header():
    ...
    def status(s):
        ...
        return h
    one ="Overview"
    two = "Spaces filled: "+str(occupancy)+"\t\t\tSpaces Available: "+str(capacity-occupancy)+"\t\tPercentage Filled: "+str(t)+"%"
    overview = Label(root,text=one,font=(None,"13","bold"),bg="grey").grid(row=0,column=0,sticky=W)
    firstline = Label(root,text=two,bg="grey").grid(row=1,column=0)
    secondline = Label(root,width=80,text="Statuses: Spaces: "+str(status("Spaces"))+", Almost Full: "+str(status("Almost"))+", Full: "+str(status(" Full"))+", Faulty: "+str(status("Faulty")),bg="grey").grid(row=2,column=0)
    #top.create_text(5,12,anchor="w",text="Overview",font=(None,"13","bold"))
    #top.create_text(5,30,anchor="w",text="Spaces filled: "+str(occupancy))
    #top.create_text(300,30,text="Spaces Available: "+str(capacity-occupancy))
    #top.create_text(460,30,anchor="w",text="Percentage Filled: "+str(t)+"%")
...
def title():
    title = Label(root,text="Car Parks",font=(None,"13","bold")).grid(row=3,column=0)
...
def bottom():
    bottom = Label(
        text="Contains Public Sector Information licensed under the Open Government License v1.0.").grid(row=11,column=0)
...
for x in range(5):
    ...
    carparklist[x].grid(row=x+4,column=0)
values()    
